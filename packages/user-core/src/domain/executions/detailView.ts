...
): ExecutionDetailStepRow[] => (steps || []).map((step) => ({
  id: step.id,
...
): ExecutionDetailPhaseRow[] => (phases || []).map((phase) => ({
  id: phase.id,
...
export const resolveExecutionInputPayload = (
  execution: ExecutionDto,
): Record<string, unknown> => execution.input || execution.normalizedInput || {};

export const resolveExecutionResultPayload = (
  execution: ExecutionDto,
): Record<string, unknown> => execution.resultJson || execution.result || {};
...
export const buildExecutionDetailActionCard = (
  ...
  if (execution.status === "human_control") {
    return {
      ...
      note: execution.takeoverReason
        ? `介入原因：${execution.takeoverReason}`
        : undefined,
      buttons: [
        {
