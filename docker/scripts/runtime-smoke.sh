...
# session-broker / browser-worker / carbone-engine / temporal / temporal-ui / temporal-sandbox-agent
...
SB_CONTAINER="${SB_CONTAINER:-ops-session-broker}"
...
main() {
  ...
  retry "session-broker running" 36 5 container_running "$SB_CONTAINER" || fail "session-broker not ready"
