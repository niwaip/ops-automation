...
mkdir -p "$SCRIPT_DIR/carbone-official/node_modules"
...
if [ "$USE_DOCKER" = false ]; then
    ...
    cd "$SCRIPT_DIR/carbone-official"
    if [ ! -d "node_modules" ]; then
        npm install --registry=https://registry.npmmirror.com
    fi
    ...
    # 保存 PID
    echo $CARBONE_PID > "$SCRIPT_DIR/.carbone.pid"
    echo $ADDIN_PID > "$SCRIPT_DIR/.addin.pid"
...
echo "  运行: $SCRIPT_DIR/load-addin.sh word"
echo ""
echo "方法2: 手动加载"
