...
以 **feature（功能模块）** 为第一组织维度，每个功能模块自洽包含 UI 组件、Hooks 和业务服务。

```
src/
├── app/                          # 应用入口、全局状态、样式
│   ├── main.tsx / App.tsx
│   ├── WordApp.tsx / ExcelApp.tsx
│   ├── TaskpaneShell.tsx
│   ├── store.ts                  # Zustand 全局状态
│   └── styles.css
│
├── host/                         # Office 宿主适配层（无业务逻辑）
│   ├── adapters/                 # HostAdapter 接口：word / excel / powerpoint
│   └── office/
│       └── word/                 # Word JS API 封装，按操作类型拆分
│           ├── word-read.*.ts    # 读取（file / structure / underline）
│           ├── word-write.api.ts
│           ├── word-highlight.api.ts
│           ├── chapter/          # 章节检测
│           └── parameter/        # 参数检测规则
│
├── api/                          # 外部 HTTP 客户端（仅负责网络请求）
│   └── carbone/
│       ├── client.ts
│       ├── template.api.ts / workflow.api.ts
│       ├── publish.api.ts / preview.api.ts
│       └── types.ts
│
├── debug/                        # 独立调试面板（不侵入业务代码）
│   ├── FlowLogPanel.tsx
│   └── flow-diagnostics.types.ts
│
├── shared/                       # 全局共享（禁止依赖 features/）
│   ├── ui/                       # 无业务依赖的通用 UI 组件
│   ├── utils/                    # host-storage、office-file-upload
│   └── services/                 # 跨 feature 共享的纯业务服务
│       └── template-source.service.ts
│
├── features/                     # ★ 业务核心，feature 为第一维度
│   ├── document-load/            # ① 文件上传 & 文档理解
│   │   ├── word/  excel/  shared/
│   ├── parameter-query/          # ② 参数查询（候选值比对）
│   │   └── word/
│   ├── parameter-identify/       # ③ AI 参数识别
│   │   ├── word/  excel/  shared/
│   │   └── services/             # 专属业务服务（不含 React）
│   │       ├── analysis-executor/    # AI 分析执行器
│   │       ├── analysis-pair-prompt/ # Excel pair 分析 prompt
│   │       ├── identify/             # 识别核心算法（excel / word）
│   │       └── suggestion/           # analyzeDocumentWithAI 入口
│   ├── draft/                    # ④ 草稿生成
│   │   ├── word/  shared/
│   ├── publish/                  # ⑤ 保存 & 导出
│   │   ├── word/  shared/
...
manifest-word.xml                 # Word 加载项清单
manifest-excel.xml                # Excel 加载项清单
manifest-ppt.xml                  # PowerPoint 加载项清单
```
...
依赖方向（单向，禁止逆向）：
  ...
  app → features → shared/services → host / api / shared/utils
