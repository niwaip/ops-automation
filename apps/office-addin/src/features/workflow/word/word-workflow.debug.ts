...
export function buildWordUnderstandingSummaryText(result: TemplateUnderstandResponse | null): string {
  if (!result) {
...
export function buildUnderstandingDebugText(result: TemplateUnderstandResponse, fallbackNarrative: string): string {
  return [
    '【理解摘要】',
...
export function buildPromptTraceDebugText(promptRequestText?: string, rawAiResponse?: string): string {
  return [
    '【发送给 AI 的完整提示词】',
    promptRequestText || '无',
    '',
