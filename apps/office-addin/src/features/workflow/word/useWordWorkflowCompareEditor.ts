...
export function useWordWorkflowCompareEditor(options: UseWordWorkflowCompareEditorOptions) {
  const persistCurrentWordCompareCacheResult = useCallback((nextResult: TemplateCompareResponse) => {
    persistWordCompareCacheResult({
      nextResult,
  ...
  const updateCompareCandidate = useCallback((candidateId: string, patch: Partial<TemplateFieldCandidate>) => {
    options.setCompareResult((current) => updateWordCompareCandidate({
      current,
      candidateId,
      patch,
    ...
    options.rebuildCompareSummary,
    options.setCompareResult,
    persistCurrentWordCompareCacheResult,
  ]);

  const deleteCompareCandidate = useCallback((candidateId: string) => {
    options.setCompareResult((current) => deleteWordCompareCandidate({
      current,
      candidateId,
      rebuildCompareSummary: options.rebuildCompareSummary,
      onPersist: persistCurrentWordCompareCacheResult,
