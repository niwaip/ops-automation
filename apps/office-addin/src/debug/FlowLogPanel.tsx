...
import { useAppStore, FlowLogEntry } from '../app/store';
...
export const FlowLogPanel: React.FC = () => {
  ...
  const isLogCollapsed = (logId: string) => collapsedLogs[logId] !== false;

  const toggleLogCollapse = (logId: string) => {
    setCollapsedLogs((prev) => ({ ...prev, [logId]: !isLogCollapsed(logId) }));
  };

  const formatLogTime = (timestamp: FlowLogEntry['timestamp']) => {
    try {
  ...
  const classifyLogModule = (log: FlowLogEntry): DebugModuleKey => {
  ...
  if (!showFlowDiagnosticsPanel) return null;

  const copyLogToClipboard = async (log: FlowLogEntry) => {
    if (!log) return;
    ...
    <div className="flow-log-panel">
      <div className="flow-log-header">
        ...
        <div className="flow-log-actions">
          ...
          <button onClick={clearFlowLogs}>清空</button>
          <button onClick={() => setShowFlowDiagnosticsPanel(false)}>关闭</button>
        </div>
      </div>
      ...
      <div className="flow-log-footer">
        ...
        <span>原始日志 {flowLogs.length} 条</span>
      </div>
    </div>
  );
};

export default FlowLogPanel;
