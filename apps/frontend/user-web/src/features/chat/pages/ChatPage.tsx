...
const normalizeMissingInputs = (
  ...
  const items = value.reduce<NonNullable<NonNullable<ChatMessage["metadata"]>["missingInputs"]>>((acc, item) => {
...
const normalizeUsage = (
  ...
  return {
    ...
    completion_tokens_details: completionDetails && typeof completionDetails.reasoning_tokens === "number"
...
const getSessionPreview = (messages: ChatMessage[] | undefined): string => {
  ...
  const preview = latestMessage.metadata?.finalSummary
...
function renderInlineRequiredInputField(
  name: string,
  type: string,
  description?: string,
) {
...
function WaitingInputInlineForm(props: WaitingInputInlineFormProps) {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      input: Object.fromEntries(requiredInputs.map((field) => [
        field.name,
  ...
  const groups = waitingInputGroups.length > 0
    ? waitingInputGroups
...
export function ChatPage({ embedded = false }: ChatPageProps) {
  ...
  const [actionLoadingByMessage, setActionLoadingByMessage] = useState<Record<string, "approve" | "reject" | undefined>>({});
  ...
  const sessions = useMemo(() => {
    ...
    return [...merged.values()].sort((left, right) => getSessionSortTime(right) - getSessionSortTime(left));
    ...
    await chatApi.stream(
      ...
      (event) => {
        ...
        updateMessage(sessionId, assistantMessageId, {
          ...
          isStreaming: event.type !== StreamEventType.RESULT
  ...
  const handleSend = () => {
    ...
    const resolvedModelId = selectedModel && selectedModel !== "default" ? selectedModel : undefined;
  ...
  const handleApprove = async (messageId: string, executionId: string) => {
    ...
    try {
      ...
      updateSessionMessages(selectedSession.id, (messages) => upsertMessage(messages, assistantMessage));
      setIsStreaming(true);
  ...
  const renderOutcomeCard = (message: ChatMessage) => {
    ...
    const title = status === "failed" || errorMessage
      ? "任务结果异常"
        ...
        {(message.metadata?.downloadUrl || message.metadata?.temporalLink || executionId) ? (
          <Space wrap className="user-chat-outcome-actions">
            ...
            {message.metadata?.temporalLink ? (
              <Button
                size="small"
                href={message.metadata.temporalLink}
                target="_blank"
              >
                查看运行详情
              </Button>
              ...
              <Space size={12} wrap>
                ...
                {usage ? (
                  <Typography.Text type="secondary" className="user-chat-usage-text">
                    Tokens: {usage.total_tokens} / 输入 {usage.prompt_tokens} / 输出 {usage.completion_tokens}
  ...
  return (
    <div className={`user-chat-page${embedded ? " embedded" : ""}${isSessionListCollapsed ? " sidebar-collapsed" : ""}`}>
        ...
        <Card className={`user-chat-status-panel${embedded ? " embedded" : ""}`}>
          <Space direction="vertical" size={12} style={{ width: "100%" }}>
            <div className="user-chat-status-heading">
              ...
              {selectedSession ? (
                <Tag className="user-chat-current-session-tag">{selectedSession.title || "新对话"}</Tag>
              ) : null}
            </div>
            {lastEvent ? (
              <Tag color={getEventTagColor(lastEvent.type)}>
                最近事件: {lastEvent.type}
              </Tag>
            ) : null}
            {modelsQuery.error ? (
              <Alert type="error" showIcon message={modelsQuery.error instanceof Error ? modelsQuery.error.message : "模型列表加载失败"} />
            ) : null}
            {sessionsQuery.error ? (
              <Alert type="error" showIcon message={sessionsQuery.error instanceof Error ? sessionsQuery.error.message : "会话列表加载失败"} />
            ) : null}
            {selectedSessionHistoryQuery.error ? (
              <Alert type="warning" showIcon message={selectedSessionHistoryQuery.error instanceof Error ? selectedSessionHistoryQuery.error.message : "历史消息加载失败"} />
            ) : null}
