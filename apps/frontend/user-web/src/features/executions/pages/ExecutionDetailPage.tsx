...
export function ExecutionDetailPage() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(['user-web-execution', id], () => executionApi.getById(id!), {
    enabled: Boolean(id),
    refetchInterval: (current) =>
      current && EXECUTION_ACTIVE_POLLING_STATUSES.includes(current.status) ? 3000 : false,
  });
  const { data: steps } = useQuery(
    ['user-web-execution-steps', id],
    ...
    return (
      <Card>
        <Typography.Text type="danger">
          {error instanceof Error ? error.message : '执行详情加载失败'}
        </Typography.Text>
      </Card>
    );
  }
  ...
  const browserExecutionResult = extractBrowserExecutionResult(
    execution.resultJson || execution.result
  );
  const displayRuntimeType = hasBrowserExecutionEvidence({
  ...
  const summaryRows = buildExecutionDetailSummaryRows(execution).map((row) =>
    row.key === 'runtime' ? { ...row, value: displayRuntimeType } : row
  );
  ...
  const releaseHumanControlMutation = useMutation(
    ...
    },
    {
      ...
      onError: (mutationError) => {
        void message.error(
          mutationError instanceof Error ? mutationError.message : '恢复自动执行失败'
        );
      },
    }
  );
    ...
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      {actionCard ? (
        <Card title={actionCard.title}>
          <Typography.Paragraph type="secondary">{actionCard.description}</Typography.Paragraph>
          {actionCard.note ? (
            <Typography.Paragraph>
              <Typography.Text strong>{actionCard.note}</Typography.Text>
            </Typography.Paragraph>
          ) : null}
          <Space>{actionCard.buttons.map((button) => renderActionButton(button))}</Space>
        </Card>
      ) : null}
      <Card>
        <Table
      ...
      {browserExecutionResult && hasBrowserAuditEvidence(browserExecutionResult) ? (
        <Card title="审计与接管证据">
          <Space direction="vertical" size={12} style={{ width: '100%' }}>
            <Space wrap>
              <Tag color={browserExecutionResult.degradedMode ? 'orange' : 'green'}>
                {`退化模式: ${browserExecutionResult.degradedMode ? '是' : '否'}`}
              </Tag>
              {browserExecutionResult.executionPlanVersion ? (
                <Tag>{`计划版本: ${browserExecutionResult.executionPlanVersion}`}</Tag>
              ) : null}
              {browserExecutionResult.runtimeEvidence?.currentRiskLevel ? (
                <Tag color="processing">{`当前风险: ${browserExecutionResult.runtimeEvidence.currentRiskLevel}`}</Tag>
              ) : null}
            </Space>
            <Table
              pagination={false}
              ...
              dataSource={[
                {
                  key: 'degradeReason',
                  label: '退化原因',
                  value: browserExecutionResult.degradeReason || '-',
                },
                {
                  key: 'currentStepId',
                  label: '当前步骤 ID',
                  value: browserExecutionResult.runtimeEvidence?.currentStepId || '-',
                },
                {
                  key: 'loopIteration',
                  label: '循环轮次',
                  value: browserExecutionResult.runtimeEvidence?.currentLoopIteration ?? '-',
                },
                {
                  key: 'riskReason',
                  label: '风险原因',
                  value: browserExecutionResult.runtimeEvidence?.riskReason || '-',
                },
                {
                  key: 'takeoverReason',
                  label: '接管原因',
                  value:
                    browserExecutionResult.runtimeEvidence?.takeoverReason ||
                    execution.takeoverReason ||
                    '-',
                },
                {
                  key: 'recorderSessionId',
                  label: '录制会话',
                  value: browserExecutionResult.trace?.recorderSessionId || '-',
                },
                {
                  key: 'exportArtifactId',
                  label: '导出产物',
                  value: browserExecutionResult.trace?.exportArtifactId || '-',
                },
                {
                  key: 'releaseId',
                  label: '发布版本',
                  value: browserExecutionResult.trace?.releaseId || '-',
                },
                {
                  key: 'skillDraftId',
                  label: '技能草稿',
                  value: browserExecutionResult.trace?.skillDraftId || '-',
                },
                {
                  key: 'publishedSkillId',
                  label: '已发布技能',
                  value: browserExecutionResult.trace?.publishedSkillId || execution.skillId || '-',
                },
                {
                  key: 'runtimeExecutionId',
                  label: '运行时执行 ID',
                  value: browserExecutionResult.trace?.runtimeExecutionId || '-',
                },
              ]}
              columns={[
            ...
            />
            {browserExecutionResult.runtimeEvidence?.lastReadValue ? (
              <div>
                <Typography.Text strong>最近读取值</Typography.Text>
                <JsonPreview value={browserExecutionResult.runtimeEvidence.lastReadValue} />
              </div>
            ) : null}
            {browserExecutionResult.runtimeEvidence?.lastBranchDecision ? (
              <div>
                <Typography.Text strong>最近分支判断</Typography.Text>
                <JsonPreview value={browserExecutionResult.runtimeEvidence.lastBranchDecision} />
              </div>
            ) : null}
          </Space>
        </Card>
      ...
      {requiredInputs.length > 0 ? (
        <Card title="待补输入">
          <Typography.Paragraph type="secondary">
            展示规则来自 user-core，表单渲染和提交由 user-web
            承接，当前页面可以直接补充输入并恢复执行。
          </Typography.Paragraph>
          <Form
            form={form}
            layout="vertical"
                    ...
                    <div key={field.name}>
                      <Form.Item
                        ...
                        rules={[
                          {
                            required: true,
                            message: `请填写 ${resolveWaitingInputDisplayLabel(field)}`,
                          },
                        ]}
                      >
      ...
      <Card title="输入">
        <JsonPreview value={resolveExecutionInputPayload(execution)} />
      </Card>
      <Card title="结果">
        <Space direction="vertical" size={12} style={{ width: '100%' }}>
          ...
          {shouldShowStructuredResult ? <JsonPreview value={resultPreviewValue} /> : null}
        </Space>
      </Card>
      <Card title="步骤">
