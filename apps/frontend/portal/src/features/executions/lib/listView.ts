...
import type { ExecutionDto, ExecutionStatus, ExecutionStepDto } from '@/api/execution';

export const formatDateTime = (date?: string) => (date ? new Date(date).toLocaleString() : '-');

export const formatListDateTime = (date?: string) => (date ? dayjs(date).format('MM-DD HH:mm') : '-');
