...
const generatePythonCode = (form: ActivityFormData): string => {
  ...
  if (form.steps.length > 0) {
    ...
    form.steps.forEach((step, idx) => {
      ...
      if (step.type === 'api') {
        ...
        lines.push(`        )`);
        lines.push(
          `        activity.logger.info(f"Step ${idx + 1} completed: {result_${idx + 1}}")`
        );
        lines.push(`    except Exception as e:`);
        lines.push(`        activity.logger.error(f"Step ${idx + 1} failed: {e}")`);
        lines.push(
          `        raise ApplicationError(f"Step ${idx + 1} failed: {e}", non_retryable=False)`
        );
      } else if (step.type === 'script') {
        lines.push(`    result_${idx + 1} = yield execute_script(`);
        lines.push('        """');
        lines.push(
          (step.config.script || '# your code here')
            .split('\n')
            .map((l: string) => `        ${l}`)
            .join('\n')
        );
  ...
  if (form.steps.length > 0) {
    ...
    form.steps.forEach((step, idx) => {
      lines.push(
        `            "${step.name}": result_${idx + 1}${idx < form.steps.length - 1 ? ',' : ''}`
      );
    });
    lines.push('        }');
  }
...
const ActivityPage: React.FC = () => {
  ...
  const [validationStrategy, setValidationStrategy] = useState<ValidationStrategy>(
  ...
  const handleEdit = async (activity: ActivityDTO) => {
    ...
    const steps: ActivityStep[] = (latestActivity.config?.steps || []).map(
      (step: ActivityStep) => ({
        ...step,
    ...
    setValidationStrategy(
      latestActivity.retryPolicy
        ? { retryPolicy: latestActivity.retryPolicy }
        : DEFAULT_VALIDATION_STRATEGY
    );
    if (latestActivity.generatedCode) {
  ...
  const columns: ColumnsType<ActivityDTO> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Tag
          color={HANDLER_CONFIG[record.handler]?.color}
  ...
  const filteredActivities = (activitiesQuery.data || []).filter(
    (a) =>
      ...
      a.fn.toLowerCase().includes(searchText.toLowerCase())
  );
  ...
  const filteredBuiltinActivities = builtinActivities.filter((activity) => {
    ...
    return [activity.name, activity.description, activity.fn, activity.ref]
      .filter(Boolean)
  ...
  return (
    <div>
        ...
        <Form layout="vertical">
          ...
          >
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Form.Item
                  label="AI 指导说明（描述这个 Activity 的功能，让 AI 生成更准确的代码）"
                  style={{ marginBottom: 0 }}
                >
                  <TextArea
                    value={activityForm.description}
                    onChange={(e) => updateActivityForm('description', e.target.value)}
                    placeholder="例如：这是一个通用查询 Activity，接收输入参数后调用外部接口并返回结果..."
                    rows={2}
                  />
                </Form.Item>
              </Col>
                  ...
                  <Panel
                    ...
                    header={
                      <Space size="middle">
                        ...
                        <Tag color={HANDLER_CONFIG[step.type]?.color}>
                          {HANDLER_CONFIG[step.type]?.label}
                        </Tag>
                      </Space>
                    }
                  >
                          ...
                          <div style={{ marginBottom: 12 }}>
                              ...
                              {normalizeInputParams(step.inputParams)
                                ...
                                .map((param, paramIdx) => (
                                  <Tag
                                    ...
                                    onMouseEnter={(e) => {
                                      e.currentTarget.style.background = param.required
                                        ? 'var(--success-color)'
                                        : 'var(--primary-color)';
                                      e.currentTarget.style.color = 'white';
                                      e.currentTarget.style.borderColor = param.required
                                        ? 'var(--success-color)'
                                        : 'var(--primary-color)';
                                    }}
                                    onMouseLeave={(e) => {
                                      e.currentTarget.style.background = param.value
                                        ? 'rgba(16, 185, 129, 0.12)'
                                        : 'var(--bg-secondary)';
                                      e.currentTarget.style.color = param.required
                                        ? 'var(--success-color)'
                                        : 'var(--text-secondary)';
                                      e.currentTarget.style.borderColor = param.required
                            ...
                            <div style={{ marginTop: 12 }}>
                                          ...
                                          <Col span={4} style={{ textAlign: 'right' }}>
                                            <Space size={4}>
                                              ...
                                              <Button
                                                ...
                                                icon={<DeleteOutlined />}
                                                onClick={() =>
                                                  removeStepInputParam(step.id, paramIdx)
                                                }
                                              />
                                            </Space>
                                          </Col>
                              ...
                              <Button
                                size="small"
                                type="dashed"
                                onClick={() => addStepInputParam(step.id)}
                                style={{ marginTop: 8 }}
                              >
                                添加输入参数
                              </Button>
          ...
          <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
            ...
            <Button
              ...
              onClick={() => {
                ...
                const allInputParams = mergedInputParams.reduce<Record<string, string>>(
                  (acc, param) => {
              ...
              disabled={!cachedCode && !generatedCode}
            >
              真实验证
            </Button>
            <Button
      ...
      <Modal
        title={
          <Space>
            <RobotOutlined /> AI 生成的 Python 代码
          </Space>
        }
        ...
        footer={[
          ...
          <Button
            ...
            type="primary"
            icon={<SaveOutlined />}
            onClick={handleCacheCode}
            loading={isCachingCode}
          >
            保存代码
          </Button>,
          <Button
      ...
      >
        <Paragraph type="secondary" style={{ marginBottom: 16 }}>
          以下代码由 AI 根据您的配置自动生成，可用于参考或复制到 Temporal Worker
          中使用。点击"缓存代码"可保存到配置中。
        </Paragraph>
        <Card
        ...
        {selectedActivity && (
          <Collapse defaultActiveKey={['basic', 'steps']} style={{ background: 'transparent' }}>
            ...
            >
              ...
              >
                ...
                <Descriptions.Item label="函数名">
                  <Tag color="cyan">{selectedActivity.fn}</Tag>
                </Descriptions.Item>
                ...
                <Descriptions.Item label="Start-To-Close">
                  {selectedActivity.timeout}
                </Descriptions.Item>
                <Descriptions.Item label="Schedule-To-Close">
                  {selectedActivity.config?.scheduleToCloseTimeout || '未设置'}
                </Descriptions.Item>
                <Descriptions.Item label="Heartbeat">
                  {selectedActivity.config?.heartbeatTimeout || '未设置'}
                </Descriptions.Item>
            ...
            <Panel
              header={
                <Text>
                  <OrderedListOutlined /> 步骤详情 ({selectedActivity.config?.steps?.length || 0})
                </Text>
              }
              key="steps"
            >
              {selectedActivity.config?.steps?.map((step: any, idx: number) => (
                ...
                >
                  ...
                  <br />
                  <Text type="secondary">
                    类型: {step.type} | 超时: {step.timeout}
                  </Text>
                  {step.type === 'api' && (
                    <div>
                      <Text type="secondary">端点: {step.config?.endpoint}</Text>
                    </div>
                  )}
                </Card>
              ))}
            ...
            >
              <Row gutter={16}>
                <Col span={8}>
                  <Card size="small">
                    <Statistic title="平均耗时" value="1.2s" />
                  </Card>
                </Col>
                <Col span={8}>
                  <Card size="small">
                ...
                <Col span={8}>
                  <Card size="small">
                    <Statistic title="Backlog" value="12" />
                  </Card>
                </Col>
              </Row>
            </Panel>
      ...
      >
        {validateMutation.isLoading ? (
          ...
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            <Text type="secondary" style={{ fontSize: 12 }}>
              说明：验证仅检查配置结构有效性，不执行生成的 Python 代码。生成的代码可直接复制到
              Temporal Worker 使用。
            </Text>
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }} size="large">
                    ...
                    <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                      {validateInputDefinitions.map((param) => (
                        ...
                        >
                          ...
                          <Input
                            placeholder={
                              param.required ? `请输入 ${param.key}（必填）` : `请输入 ${param.key}`
                            }
                            value={validateInputParams[param.key] ?? ''}
                            onChange={(e) =>
                              setValidateInputParams((prev) => ({
                                ...prev,
          ...
          {realValidateState.result &&
            !realValidateState.error &&
            realValidateState.result.result && (
              <Collapse
                defaultActiveKey={[]}
                style={{ background: 'transparent' }}
                items={[
                  {
                    ...
                    children: (
                      ...
                      >
                        ...
                        >
                          {typeof realValidateState.result.result === 'string'
                            ? realValidateState.result.result
                            : JSON.stringify(realValidateState.result.result, null, 2)}
                        </pre>
                      </Card>
                    ),
                  },
