...
const parseCliHtmlSummary = (rawHtml?: string): {
  ...
  const parseMaybeJson = (value?: string): unknown => {
    ...
    try {
      const firstPass: unknown = JSON.parse(trimmed);
...
const SessionListPage: React.FC = () => {
  ...
  const handleGenerateReport = async () => {
    if (!selectedSessionId || !selectedTemplateId) {
      void message.warning('Please select a template');
      return;
    }
    ...
    try {
      ...
      void message.success('Report generation started');
      setReportModalVisible(false);
      navigate(`/reports/${result.report_id}`);
    } catch {
      void message.error('Failed to generate report');
    } finally {
