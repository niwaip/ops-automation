...
export const capabilityReleaseApi = {
  list: async (): Promise<{ releases: CapabilityRelease[] }> => {
    return apiClient.get<{ releases: CapabilityRelease[] }>('/capabilities');
  },

  listReleaseCenter: async (): Promise<{ releases: CapabilityRelease[] }> => {
    return apiClient.get<{ releases: CapabilityRelease[] }>('/capabilities/release-center');
  },

  getById: async (id: string): Promise<{ release: CapabilityReleaseDetail }> => {
    return apiClient.get<{ release: CapabilityReleaseDetail }>(`/capabilities/${id}`);
  },
  ...
  create: async (data: {
    ...
    return apiClient.post<{ release: CapabilityReleaseDetail }>('/capabilities', data);
  },
  ...
  buildStream: async (
    ...
    const response = await fetch(
      `${baseUrl}/capabilities/${id}/build/stream?${params.toString()}`,
      {
        method: 'GET',
        headers: {
  ...
  validateSandboxStream: async (
    ...
    const response = await fetch(
      `${baseUrl}/capabilities/${id}/validate/sandbox/stream?${params.toString()}`,
      {
        method: 'GET',
        headers: {
          Accept: 'text/event-stream',
  ...
  getCurrentSkillDraft: async (id: string): Promise<{ skillDraft: SkillDraft }> => {
    return apiClient.get<{ skillDraft: SkillDraft }>(`/capabilities/${id}/skill-draft`);
  },
  ...
  suggestWizardAssist: async (
    ...
    return apiClient.post(`/capabilities/${id}/wizard-assist`, data);
  },

  getDeployments: async (id: string): Promise<{ deployments: DeploymentRecord[] }> => {
    return apiClient.get<{ deployments: DeploymentRecord[] }>(`/capabilities/${id}/deployments`);
  },

  getAuditEvents: async (id: string): Promise<{ auditEvents: ReleaseAuditEvent[] }> => {
    return apiClient.get<{ auditEvents: ReleaseAuditEvent[] }>(`/capabilities/${id}/audit-events`);
  },
  ...
  archive: async (id: string): Promise<{ success: true; archivedId: string }> => {
    return apiClient.delete<{ success: true; archivedId: string }>(`/capabilities/${id}`);
  },
  ...
  analyzeFailure: async (
    ...
    return apiClient.post(`/capabilities/${id}/analyze-failure`, data);
  },
};
