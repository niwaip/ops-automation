...
void Application::HandleActivationDoneEvent() {
    ESP_LOGI(TAG, "AI Passport Activation done");

    SystemInfo::PrintHeapStats();
    SetDeviceState(kDeviceStateIdle);

    auto display = Board::GetInstance().GetDisplay();
    display->ShowNotification("AI Passport 就绪");
    display->SetChatMessage("system", "AI Passport 智能运维终端\n长按语音键下发任务");
    display->SetStatus("AI Passport 就绪");
...
void Application::ActivationTask() {
    ...
    // Initialize the AI Passport MQTT protocol directly
    InitializeProtocol();
...
void Application::InitializeProtocol() {
    ...
    display->SetStatus("正在连接 EMQX 运维网关...");

    // Directly use MqttProtocol for AI Passport
    protocol_ = std::make_unique<MqttProtocol>();

    protocol_->OnConnected([this]() {
        DismissAlert();
        auto display = Board::GetInstance().GetDisplay();
        display->SetStatus("AI Passport 就绪 (EMQX已连接)");
    });
...
void Application::HandleWakeWordDetectedEvent() {
    // Disabled in AI Passport: task dispatch is initiated via PTT button
    return;
}
...
void Application::HandleStateChangedEvent() {
    ...
    switch (new_state) {
        ...
        case kDeviceStateConnecting:
            display->SetStatus("正在连接 EMQX...");
            display->SetEmotion("neutral");
            break;
        case kDeviceStateListening:
            display->SetStatus("正在录音 (松开提交)...");
            display->SetEmotion("neutral");
            display->SetChatMessage("system", "【正在录音】松开按键提交运维任务");
            StartListeningAudio();
            break;
