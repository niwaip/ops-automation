...
export class BrowserStepService {
  ...
  async buildSteps(
    ...
    const baseSteps = commands.map((command, index) => this.browserStepMapper.toActionStep({
      command,
    ...
    const locatorEnrichedSteps = !adapter?.generateLocator
      ? baseSteps
      : await Promise.all(baseSteps.map(async (step) => {
      if (step.status !== 'success' || !step.runtimeTargetRef) {
        return step;
      }

      try {
        const locatorExpression = await adapter.generateLocator?.(step.runtimeTargetRef, options);
    ...
    return locatorEnrichedSteps.map((step) => this.browserParameterizationService.parameterizeStep(step));
  }
}
