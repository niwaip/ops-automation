...
export class CodegenScriptParserService {
  ...
  private parseLine(
    ...
    if (
      ...
      return this.buildStep(index, 'switch_latest_tab', {
        params: {},
        scriptFragment: line,
      }, context);
    }
    ...
    if (gotoMatch?.[2]) {
      return this.buildStep(index, 'navigate', {
    ...
    const pageClickMatch = line.match(
      new RegExp(`^await\\s+${this.escapeForRegex(pageAlias)}\\.click\\((['"])([\\s\\S]*?)\\1\\);?$`),
    );
    if (pageClickMatch?.[2]) {
      const selector = pageClickMatch[2];
      return this.buildStep(index, 'click', {
        params: { selector },
    ...
    if (pageFillMatch?.[2] && pageFillMatch[4] !== undefined) {
      ...
      return this.buildStep(index, 'fill', {
    ...
    const pageHoverMatch = line.match(
      new RegExp(`^await\\s+${this.escapeForRegex(pageAlias)}\\.hover\\((['"])([\\s\\S]*?)\\1\\);?$`),
    );
    if (pageHoverMatch?.[2]) {
      const selector = pageHoverMatch[2];
      return this.buildStep(index, 'hover', {
        params: { selector },
        locator: { strategy: 'css', value: selector },
        scriptFragment: line,
      }, context);
    ...
    if (keyboardPress?.[2]) {
      return this.buildStep(index, 'press_key', {
