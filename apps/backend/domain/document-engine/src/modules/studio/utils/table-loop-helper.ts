...
export function inferTableArrayPath(
  headerRow: string,
  templateType: string,
  tableIndex: number
): string {
    ...
    if (elementIndex >= 0 && elementIndex < elements.length) {
      ...
      if (tableElement && tableElement.type === 'table') {
        ...
        if (
          ...
          // 使用表格结构中的表头信息生成列映射
          columnMappings = generateColumnMappingsFromHeaders(
            tableElement,
            loop.arrayPath || 'd.items'
          );
        } else {
          // 规范化AI返回的列映射
 ...
 * 从表格结构生成列映射
 */
export function generateColumnMappingsFromHeaders(
...
export function expandColumnMappingsForHeader(
  rawHeader: string,
  arrayPath: string,
  columnIndex: number,
  ...
  return headerSegments.map((header, segmentIndex) => {
    ...
    const varName = buildTableLoopVarName(derivedBaseName, language, usedNames, segmentIndex);
...
export function buildTableLoopVarName(
  baseName: string,
