...
import {
  TemplateConfig, VariableMapping, TableLoop, CombinedVariable, GroupLoop, PathMappingRule, DEFAULT_PATH_MAPPINGS
} from './types';
import { validateTableLoops } from './table-loop-helper';
import { validateGroupLoops, validateCombinedVariables, validateVariableMappings, extractFormatter, generateFallbackSuggestions } from './parameter.helper';
...
const logger = new Logger('StudioAiPromptBuilder');
    ...
    try {
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        try {
          ...
          const batchSuggestions = parseAIResponseToSuggestions(aiResponse, batchPatterns, batchIndex * batchSize);
          ...
          const fallbackSuggestions = generateFallbackSuggestions(batchPatterns, templateType, batchIndex * batchSize);
   ...
   * AI建议优先，规则建议补充缺失的部分
   */
export function mergeSuggestions(aiSuggestions: any[], fallbackSuggestions: any[], patterns: any[]) : any[] {
...
export function buildAIPromptForBlanks(patterns: Array<{ text: string; context: string; beforeBlank?: string; position: number; type: string }>,
    fullContent: string,
    templateType: string,
    context?: string,
...
export function parseAIResponseToSuggestions(aiResponse: any,
    ...
    for (const aiSuggestion of aiResponse.suggestions) {
      ...
      if (patternIndex < 0 || patternIndex >= patterns.length) {
        logger.warn(`AI suggestion index ${aiSuggestion.index} out of range for batch (start=${startIndex}, size=${patterns.length})`);
        continue;
      }
...
export function buildAIAnalysisPrompt(elements: DocumentElement[],
    context?: string,
    manualMarkings?: Record<string, string>,
    markingSummary?: string) : string {
    // 构建文档元素摘要（简化版）
...
- 只返回JSON，不要解释`;
   ...
   * 处理AI生成的参数对照表和模版配置
   */
export function parseAIAnalysisResponse(response: string, elements: DocumentElement[]) : TemplateConfig {
    try {
      // 尝试提取 JSON
      ...
      const config: TemplateConfig = {
        ...
        variableMappings: validateVariableMappings(rawVariableMappings, elements, mergedMappings, staticElements),
...
export function parseAIParameterMappings(aiMappings: any[]) : PathMappingRule[] {
   ...
   * AI生成的规则优先（排在前面）
   */
export function mergePathMappings(aiMappings: PathMappingRule[], defaultMappings: PathMappingRule[]) : PathMappingRule[] {
