...
export * from './skill-parameter.helper';

const logger = new Logger('StudioParameterHelper');

export function generateFallbackSuggestions(patterns: Array<{ text: string; context: string; beforeBlank?: string; position: number; type: string; chapter?: string; significance?: string }>,
    templateType: string,
    startIndex: number = 0) : any[] {
    const suggestions: any[] = [];
    logger.debug(`Generating suggestions for ${patterns.length} patterns, templateType: ${templateType}`);
        ...
        for (const [mappingLabel, mapping] of Object.entries(labelMappings)) {
          if (label === mappingLabel || label.includes(mappingLabel) || mappingLabel.includes(label)) {
            if (!usedPaths.has(mapping.path)) {
              ...
              logger.debug(`Pattern ${i}: matched label "${label}" -> ${suggestedPath} (confidence: ${confidence})`);
              break;
            }
          }
        }
...
export * from './template-config.helper';
