...
export class TemplateRepository {
  ...
  private buildUpsertData(filePath: string, meta: TemplateMetaDocument): Prisma.TemplateUncheckedCreateInput {
    ...
    const loops = Array.isArray(meta.loops) && meta.loops.length > 0
      ? meta.loops
    ...
    const type = meta.type === 'marked_template' ? TemplateType.marked_template : TemplateType.template;
    ...
    return {
      ...
      ignoredElements: (meta.ignoredElements ?? Prisma.DbNull) as Prisma.InputJsonValue | Prisma.NullTypes.DbNull,
      elementGroups: (meta.elementGroups ?? Prisma.DbNull) as Prisma.InputJsonValue | Prisma.NullTypes.DbNull,
      ignoredGroups: (meta.ignoredGroups ?? Prisma.DbNull) as Prisma.InputJsonValue | Prisma.NullTypes.DbNull,
      markingsSavedAt: meta.savedAt ? new Date(meta.savedAt) : null,
      templateConfig: (templateConfig ?? Prisma.DbNull) as Prisma.InputJsonValue | Prisma.NullTypes.DbNull,
      configSavedAt: meta.configSavedAt ? new Date(meta.configSavedAt) : null,
      suggestions: (suggestions.length > 0 ? suggestions : Prisma.DbNull) as Prisma.InputJsonValue | Prisma.NullTypes.DbNull,
      verifyResult: (meta.verifyResult ?? Prisma.DbNull) as Prisma.InputJsonValue | Prisma.NullTypes.DbNull,
  ...
  private mapTemplate(template: {
    ...
    return {
      ...
      elementGroups: this.isRecord(template.elementGroups) ? template.elementGroups as Record<string, number[]> : undefined,
      ignoredGroups: (Array.isArray(template.ignoredGroups) ? template.ignoredGroups : undefined) as string[] | undefined,
      ...
      verifyResult: this.isRecord(template.verifyResult) ? template.verifyResult as TemplateResponse['verifyResult'] : undefined,
  ...
  private extractLoopsFromSuggestions(suggestions: unknown[]): Array<{ arrayPath: string }> {
    return suggestions
      ...
      .map((item) => {
        ...
        const arrayPath = details && typeof details.arrayPath === 'string' && details.arrayPath.trim().length > 0
