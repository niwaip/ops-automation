...
```
services/carbone-engine/
├── src/modules/studio/
│   └── ai-identifier.service.spec.ts    # 单元测试
├── src/test/e2e/
...
# 运行测试（使用mock AI服务器）
docker compose -f docker-compose.test.yml up --abort-on-container-exit carbone-engine-test

# 或者单独运行测试
docker compose -f docker-compose.test.yml run --rm carbone-engine-test
```
...
# 运行单元测试
npm run test -- --testPathPattern="ai-identifier.service.spec"
