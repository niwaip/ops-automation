...
const logger = new Logger('StudioTemplateConfigHelper');
...
export function analyzeContentPattern(text: string) : ContentPattern {
  ...
  */
export function generateTemplateConfig(elements: DocumentElement[], userIntent: UserIntent) : TemplateConfig {
...
export function generateVariablePath(el: DocumentElement) : string {
  ...
  * 检测变量类型
  */
export function detectVariableType(el: DocumentElement) : 'text' | 'number' | 'date' | 'image' | 'heading' {
...
export function validateVariableMappings(mappings: any[],
    elements: DocumentElement[],
    pathMappings?: PathMappingRule[],
    staticElements?: StaticElement[]) : VariableMapping[] {
   ...
   * @returns 规范化后的标准路径
   */
export function normalizeVariablePath(originalPath: string, pathMappings?: PathMappingRule[]) : string {
    // 使用传入的对照表或默认对照表
    ...
    for (const rule of mappings) {
      ...
      for (const pattern of rule.patterns) {
        if (matchPathPattern(originalPath, pattern)) {
          logger.debug(`Path mapping (pattern): ${originalPath} -> ${rule.standardPath} (${rule.description})`);
          return rule.standardPath;
        }
      }
    }
...
export function shouldSkipProtectedTitleVariableMapping(path: string,
    element: DocumentElement,
    mapping: any,
    protectedTitleTexts: Set<string>) : boolean {
...
export function validateGroupLoops(groupLoops: any[]) : GroupLoop[] {
   ...
   */
export function validateCombinedVariables(combinedVars: any[], elements: DocumentElement[]) : CombinedVariable[] {
