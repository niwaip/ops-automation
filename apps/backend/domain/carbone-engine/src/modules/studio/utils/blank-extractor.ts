...
export function mergeUnderlineInfo(existingBlanks: Array<{
      text: string;
      context: string;
      beforeBlank: string;
      position: number;
    ...
    for (const underline of underlineInfo) {
      ...
      logger.debug(`下划线 #${result.length + 1}: "${underline.text.substring(0, 20)}..." (${underline.text.length}字符) - hasBlank:${!!hasBlank}, isBlank:${isBlankUnderline}, duplicate:${isDuplicate}`);
      ...
      if (!isDuplicate) {
        ...
        // 判断意义
        const significance = getSignificanceForLabel(label, templateType) ||
          '带下划线的空白位置，用于填写相关内容';
...
export function extractBlankPatterns(content: string, templateType: string) : Array< {
    text: string;
    ...
    while ((match = underlineRegex.exec(content)) !== null) {
      ...
      const significance = getSignificanceForLabel(beforeBlank, templateType) ||
        '带下划线的空白位置，需要填写相关内容';
   ...
   * 用于精确定位空白所在位置
   */
export function extractChapterStructure(content: string) : Array< { title: string; startPos: number; endPos: number }> {
   ...
   * 根据位置获取所在章节
   */
export function getChapterForPosition(position: number, chapters: Array<{ title: string; startPos: number; endPos: number }>) : string {
