...
const BROWSER_ROOT_NODE_ID = 'browser_recording';
...
export class RecorderCompositePlanCompilerService {
  ...
  async compile(value: unknown): Promise<DeterministicPlanDraftV1> {
    ...
    for (const post of spec.composition.postProcessingSteps) {
      ...
      if (
        ...
        !post.inputBindings &&
        !dependsOn.includes(BROWSER_ROOT_NODE_ID)
      ) {
        dependsOn.push(BROWSER_ROOT_NODE_ID);
      }
      if (post.type === 'llm_operation') {
        ...
        if (!ref || !matchesVersion) {
          throw new BadRequestException(`RECORDER_COMPOSITION_OPERATION_VERSION_INVALID: ${post.operationId}@${post.operationVersion}`);
        }
        nodes.push({
          nodeId: post.id,
          sequence,
          ...
          dependsOn,
          inputBindings: this.normalizeNodeBindings(
            post.inputBindings,
            contentOutputs,
            dependsOn,
          ),
          ...
          inputBindings: post.inputBindings
            ...
            : {
                report: {
                  source: 'node_output',
                  nodeId: BROWSER_ROOT_NODE_ID,
                  fromNodeId: BROWSER_ROOT_NODE_ID,
    ...
    const finalOutputs = primary ? [{
      targetField: 'result', fromNodeId: finalNode.nodeId, fromNodeOutput: primary,
      expectedType: finalNode.outputContract[primary]!, isArtifact: finalNode.outputContract[primary] === 'artifact_ref',
    }] : [];
    const plan: DeterministicPlanDraftV1 = {
      schemaVersion: 'deterministic-plan/v1', plannerVersion: 'recorder-composition/v1', catalogVersion: 'runtime',
      ...
      status: 'validated', nodes, finalOutputs,
    };
    plan.planHash = createHash('sha256').update(JSON.stringify(canonicalizePlan(plan))).digest('hex');
    return plan;
  }
  ...
  private normalizeNodeBindings(
    ...
    for (const [field, value] of Object.entries(raw || {})) {
      ...
      const fallbackPath =
        sourceNodeId === BROWSER_ROOT_NODE_ID && contentOutputs.length === 1
          ? contentOutputs[0]
          : undefined;
      const rawPath = typeof binding.path === 'string' ? binding.path : typeof binding.outputPath === 'string' ? binding.outputPath : fallbackPath;
      ...
      if (!rawPath && paths.length === 0) {
        throw new BadRequestException(`RECORDER_COMPOSITION_CONTENT_BINDING_AMBIGUOUS: field '${field}' needs an explicit content output path`);
      }
      bindings[field] = {
        source: 'node_output', nodeId: sourceNodeId, fromNodeId: sourceNodeId,
        path: rawPath || paths.join(','),
        ...(paths.length > 1 ? { paths } as any : {}),
        ...(typeof binding.expectedType === 'string' ? { expectedType: binding.expectedType as any } : { expectedType: 'json' }),
  ...
  private validate(value: unknown): RecorderCompositeExecutionSpecV1 {
    const spec = value && typeof value === 'object' && !Array.isArray(value)
      ...
      : undefined;
    if (!spec?.browser?.skillId || !spec.browser.skillVersion || !spec.composition || !Array.isArray(spec.composition.outputDeclarations) || !Array.isArray(spec.composition.postProcessingSteps)) {
      throw new BadRequestException('RECORDER_COMPOSITION_INVALID: browser skill/version, output declarations and post-processing steps are required');
    }
    if (spec.composition.postProcessingSteps.length === 0) {
      throw new BadRequestException('RECORDER_COMPOSITION_INVALID: at least one explicit post-processing step is required');
    }
    const postIds = spec.composition.postProcessingSteps.map((post) => post.id?.trim());
    if (
      postIds.some((id) => !id || id === BROWSER_ROOT_NODE_ID) ||
      new Set(postIds).size !== postIds.length
    ) {
      throw new BadRequestException(
        `RECORDER_COMPOSITION_INVALID: post-processing IDs must be unique, non-empty, and cannot use '${BROWSER_ROOT_NODE_ID}'`,
      );
    }
    return spec;
  }
