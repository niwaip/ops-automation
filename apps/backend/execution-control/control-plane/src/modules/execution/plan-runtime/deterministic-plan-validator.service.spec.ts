...
describe('DeterministicPlanValidatorService', () => {
  ...
  it('should validate a plan with built-in capability key platform.document.markdown-artifact-writer', () => {
    const builtinPlan: DeterministicPlanDraftV1 = JSON.parse(JSON.stringify(valid3NodePlan));
    (builtinPlan.nodes[2] as any).publishedSkillId = 'platform.document.markdown-artifact-writer';
    const result = validator.validatePlan(builtinPlan);
    expect(result.valid).toBe(true);
    expect(result.errors).toHaveLength(0);
  });
