...
export class XlsxSharedStringsService {
  constructor(
    private readonly builder: Builder,
    private readonly preprocessor: XmlPreprocessor
  ) {}

  async processSharedStrings(zip: JSZip, data: any): Promise<void> {
    ...
    if (sharedStringsFile) {
      ...
      zip.file(sharedStringsPath, result.xml);
    }
  }

  async expandSharedStringLoopRows(
    zip: JSZip,
    data: any,
  ...
  private async getOptionalFileContent(zip: JSZip, filePath: string): Promise<string | null> {
