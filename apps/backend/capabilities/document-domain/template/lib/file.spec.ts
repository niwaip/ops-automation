...
describe('FileHandler', () => {
  ...
  it('expands shared string loop rows beyond placeholders and updates formula ranges', () => {
    const service = new XlsxSharedStringsService(new Builder(), new XmlPreprocessor()) as any;
    ...
    const result = service.expandWorksheetSharedStringLoops(
      sheetXml,
      originalStrings,
  ...
  it('updates linked worksheet tables for expanded row ranges', async () => {
    ...
    await service.updateWorksheetTables(
      zip,
      new Map([
  ...
  it('forces workbook recalculation for regenerated xlsx files', async () => {
    ...
    await service.enableWorkbookRecalculation(zip);
