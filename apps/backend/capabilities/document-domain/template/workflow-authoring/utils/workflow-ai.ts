...
import { buildFallbackSectionHints } from './workflow-similarity';
...
export async function prepareWorkflowUnderstandingContext(input: {
  templateDocumentIr: WorkflowDocumentIR;
  sampleDocument?: { fileName?: string; contentBase64?: string };
  targetLanguages: string[];
  ...
  const elements = Array.isArray(input.templateDocumentIr.elements)
    ? input.templateDocumentIr.elements
    : [];
  ...
  return {
    paragraphCount,
    tableCount,
    fallbackSectionHints: buildFallbackSectionHints(elements),
    fallbackTerminologyCandidates: Array.from(
      new Set(
        input.compareCandidates
          .flatMap((candidate) => [
            safeText(candidate.sampleValue),
            safeText(candidate.anchorText),
            safeText(candidate.sectionTitle),
          ])
          .filter((text) => Boolean(text) && text.length <= 40)
      )
    ).slice(0, 8),
    fallbackLayoutFeatures: [
      paragraphCount > 0 ? `paragraphs:${paragraphCount}` : '',
      tableCount > 0 ? `tables:${tableCount}` : '',
      input.targetLanguages.length > 0 ? `targets:${input.targetLanguages.join(',')}` : 'single_language',
      input.sampleDocument?.contentBase64 ? 'sample_attached' : 'template_only',
    ].filter(Boolean),
    fieldCandidateIds: input.compareCandidates.map(
      (candidate) => candidate.fieldIdHint || candidate.candidateId
    ),
    sampleText: await extractSampleTextRich(
      input.sampleDocument?.contentBase64,
      input.warnings
    ),
  };
}

export function buildWorkflowUnderstandResultSummary(input: {
  templateDocumentIr: WorkflowDocumentIR;
  sampleDocument?: { fileName?: string; contentBase64?: string };
  sourceLanguage: string;
  targetLanguages: string[];
...
}): WorkflowUnderstandResult['summary'] {
  const documentTitle =
    safeText(input.understandingSummary.documentTitle) ||
  ...
  return {
    ...
    understandingSummaryText:
      safeText(input.understandingSummary.understandingSummaryText) ||
      buildFallbackWorkflowUnderstandingSummaryText({
        ...
        sourceLanguage: input.sourceLanguage,
        targetLanguages: input.targetLanguages,
        paragraphCount: input.understandingInput.paragraphCount,
        tableCount: input.understandingInput.tableCount,
        sectionHints: input.understandingSummary.sectionHints,
        terminologyCandidates: input.understandingSummary.terminologyCandidates,
        layoutFeatures: input.understandingSummary.layoutFeatures,
        fieldCandidateIds: input.fieldIds,
    ...
    tableCount: input.understandingInput.tableCount,
    sectionHints: input.understandingSummary.sectionHints,
    sectionSummaries: input.understandingSummary.sectionSummaries,
    terminologyCandidates: input.understandingSummary.terminologyCandidates,
    fieldCandidateIds: input.understandingInput.fieldCandidateIds,
    layoutFeatures: input.understandingSummary.layoutFeatures,
  };
}

export function buildWorkflowUnderstandResultMeta(input: {
  warnings: string[];
  compareCandidateWarnings?: string[];
  understandingSummaryWarnings: string[];
...
}): Pick<WorkflowUnderstandResult, 'warnings' | 'contextAnalysis'> {
  return {
    warnings: Array.from(
      new Set([
        ...input.warnings,
        ...(input.compareCandidateWarnings || []),
        ...input.understandingSummaryWarnings,
      ])
    ),
    contextAnalysis: {
      usedAI: input.understandingContext.usedAI,
      aiServiceUrl: input.understandingContext.aiServiceUrl,
      promptRequestText: input.understandingContext.promptRequestText,
      rawAiResponse: input.understandingContext.rawAiResponse,
    },
  };
}

export function buildWorkflowUnderstandResult(input: {
  analysisId: string;
  languageProfile: WorkflowUnderstandResult['languageProfile'];
  templateDocumentIr: WorkflowDocumentIR;
  sampleDocument?: { fileName?: string; contentBase64?: string };
  sourceLanguage: string;
  targetLanguages: string[];
  ...
  const meta = buildWorkflowUnderstandResultMeta({
    warnings: input.warnings,
    compareCandidateWarnings: input.compareCandidateWarnings,
    understandingSummaryWarnings: input.understandingSummary.warnings,
  ...
  return {
    ...
    summary: buildWorkflowUnderstandResultSummary({
      templateDocumentIr: input.templateDocumentIr,
      sampleDocument: input.sampleDocument,
      sourceLanguage: input.sourceLanguage,
      targetLanguages: input.targetLanguages,
      fieldIds: input.fieldIds,
      understandingInput: input.understandingInput,
      understandingSummary: input.understandingSummary,
    }),
    warnings: meta.warnings,
    contextAnalysis: meta.contextAnalysis,
  };
}
