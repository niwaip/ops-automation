...
export function buildVariableMappingsFromSuggestions(suggestions: any[]): VariableMapping[] {
  return suggestions.reduce<VariableMapping[]>((acc, suggestion, idx) => {
    ...
    acc.push({
      ...
      index: idx,
      type: (suggestion?.details?.fieldType ||
        suggestion?.fieldType ||
        'text') as VariableMapping['type'],
...
export function resolveSuggestionGroupMeta(suggestion: any): {
  groupLabel?: string;
  sheetName?: string;
} {
...
export function inferFieldType(name: string, originalText: string): string {
  ...
  if (
    ...
    return 'amount';
  if (lowerName.includes('电话') || lowerName.includes('phone') || lowerName.includes('手机'))
    return 'phone';
  ...
  if (lowerName.includes('编号') || lowerName.includes('number') || lowerName.includes('code'))
    return 'code';
  if (lowerName.includes('数量') || lowerName.includes('count') || lowerName.includes('qty'))
    return 'number';
  if (
    lowerName.includes('名称') ||
    lowerName.includes('name') ||
    lowerText.includes('甲方') ||
    lowerText.includes('乙方')
  )
    return 'name';
 ...
 * 生成提取提示
 */
export function generateExtractionHint(
  name: string,
  fieldType: string,
  originalText: string,
  templateType: string
...
export function normalizeSkillParameterPath(variableName: string): string {
  ...
  if (explicitLoopMatch?.[2]) {
    return explicitLoopMatch[2].replace(/\[(?:i(?:\+\d+)?)|\d+\]/g, '[]').trim();
  }

  const markerMatch = rawValue.match(/\{([cdt])\.([^}:]+)(?::[^}]*)?\}/);
  if (markerMatch?.[2]) {
    return markerMatch[2].replace(/\[(?:i(?:\+\d+)?)|\d+\]/g, '[]').trim();
  }
...
export function isPlaceholderSkillParameterPath(variableName: string): boolean {
  ...
  return normalized.split('.').some((segment) => /^field(?:[_-]?\d+)?$/i.test(segment.trim()));
}
 ...
 */
export function buildSkillExampleValue(
...
export function generateExampleValue(fieldType: string, variableName: string): string {
  ...
  switch (fieldType) {
    ...
    default:
      ...
      if (lastPart.includes('地址') || lastPart.includes('address'))
        return '北京市朝阳区望京东路 1 号';
      if (lastPart.includes('日期') || lastPart.includes('date') || lastPart.includes('year'))
        return '2026';
 ...
 * 构建完整的AI使用指导
 */
export function buildCompleteAIInstructions(
  templateType: string,
  parameters: any[],
  documentDescription?: string
): string {
  const parameterRules = parameters
    .map(
      (p) =>
        `- ${p.name}: ${p.usage || '填写对应值'}；类型=${p.dataType}；提取提示=${p.extractionHint || '结合上下文提取'}；示例=${p.example}`
    )
    .join('\n');
 ...
 * 用于展示如何构建可用的数据结构
 */
export function buildDataExampleJson(
  parameters: any[],
  ...
  const setValueAtPath = (target: Record<string, any>, rawPath: string, value: unknown) => {
    ...
    for (let i = 0; i < pathParts.length; i++) {
      ...
      if (isArrayPart) {
        ...
        if (
          !current[key][arrayIndex] ||
          typeof current[key][arrayIndex] !== 'object' ||
          Array.isArray(current[key][arrayIndex])
        ) {
          current[key][arrayIndex] = {};
        }
  ...
  for (const loop of tableLoops) {
    if (Array.isArray(loop.columnMappings)) {
      for (const column of loop.columnMappings) {
        ...
        const sampleValue =
          sanitizeSkillExampleSource(column?.sampleValue) ||
...
export function buildLoopExampleValueForIndex(
  baseValue: unknown,
  fieldType: string,
  variablePath: string,
  index: number
  ...
  const base =
    sanitizeSkillExampleSource(baseValue) || generateExampleValue(fieldType, variablePath);
  if (index <= 0) {
    return base;
  }
  ...
  if (
    /(^|\.)(seq|serialno|serialnumber|lineno|lineno|lineNo)$/i.test(cleanPath) ||
    lastPart === 'seq'
  ) {
...
export function extractLoopColumnMappings(
  suggestion: any,
  tableLoops: Array<Partial<TableLoop>>,
  arrayPath: string
): ColumnMapping[] {
  if (
    Array.isArray(suggestion?.details?.columnMappings) &&
    suggestion.details.columnMappings.length > 0
  ) {
  ...
  const matchedLoop = tableLoops.find(
    (loop) => String(loop.arrayPath || '').trim() === normalizedArrayPath
  );
  return Array.isArray(matchedLoop?.columnMappings)
    ? (matchedLoop.columnMappings as ColumnMapping[])
    : [];
}
export function buildSkillTableLoops(suggestions: any[], templateConfig: any): TableLoop[] {
  ...
  const addLoop = (loop: Partial<TableLoop> | null | undefined) => {
    ...
    loopMap.set(arrayPath, {
      tableIndex:
        loop?.tableIndex !== undefined ? Number(loop.tableIndex) : (existing?.tableIndex ?? -1),
      headerRow: String(loop?.headerRow || existing?.headerRow || ''),
      dataRowCount:
        loop?.dataRowCount !== undefined
          ? Number(loop.dataRowCount)
          : (existing?.dataRowCount ?? 0),
      arrayPath,
      columnMappings:
        normalizedColumnMappings.length > 0
          ? normalizedColumnMappings
          : existing?.columnMappings || [],
      reason: String(loop?.reason || existing?.reason || '根据循环建议推导的表格循环'),
      confidence:
        loop?.confidence !== undefined ? Number(loop.confidence) : (existing?.confidence ?? 0.9),
    });
  };
  ...
  for (const suggestion of suggestions) {
    ...
    const arrayPath = String(
      suggestion?.details?.arrayPath || suggestion?.suggestedName || ''
    ).trim();
    ...
    addLoop({
      ...
      dataRowCount: 1,
      reason: String(
        suggestion?.details?.significance || suggestion?.context || '来自前端循环建议'
      ),
      confidence: suggestion?.confidence !== undefined ? Number(suggestion.confidence) : 0.9,
      columnMappings: Array.isArray(suggestion?.details?.columnMappings)
 ...
 */
export function buildSkillGuideMarkdown(
  templateType: string,
  ...
  try {
    parsedExample =
      typeof dataExampleJson === 'string' ? JSON.parse(dataExampleJson) : dataExampleJson;
  } catch (e) {
