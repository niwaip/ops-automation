...
export class BuiltinSkillController {
  ...
  private isTrustedInternalCaller(req: Request): boolean {
    const internalSecret = process.env.INTERNAL_API_SECRET || process.env.INTERNAL_API_SHARED_SECRET;
    const hasUser = Boolean((req as any).user);
    if (!internalSecret) {
      // No secret configured — only authenticated JWT callers are trusted
      return hasUser;
    }
    const reqSecret = req.headers['x-internal-secret'] || req.headers['x-internal-token'] || req.headers['x-internal-auth'];
  ...
  async getUnifiedCatalog(@Req() req: Request) {
    ...
    const orgId = user?.activeOrgId || user?.orgId || (isTrusted ? (req.headers['x-org-id'] as string) : undefined);
    ...
    const catalog = await this.catalogProjectionService.getUnifiedCatalog({ userId, orgId, roleIds });
  ...
  async resolveCapability(@Body() dto: ResolveRequestDto, @Req() req: Request) {
    ...
    const userId = isTrusted && dto.userId ? dto.userId : user?.id || (isTrusted ? (req.headers['x-user-id'] as string) : undefined);
    const orgId = isTrusted && dto.orgId ? dto.orgId : user?.activeOrgId || user?.orgId || (isTrusted ? (req.headers['x-org-id'] as string) : undefined);
    const roleIds = isTrusted && dto.roleIds ? dto.roleIds : (Array.isArray(user?.roles) ? user.roles : isTrusted && req.headers['x-role-ids'] ? String(req.headers['x-role-ids']).split(',') : undefined);
