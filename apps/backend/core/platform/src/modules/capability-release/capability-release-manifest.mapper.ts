...
function readExecutionFlow(detail: CapabilityReleaseDetailDTO): Array<Record<string, unknown>> {
  ...
  if (Array.isArray(draft?.executionFlow)) {
    return draft.executionFlow.filter((item): item is Record<string, unknown> => Boolean(asRecord(item)));
  }
...
function buildRuntimeRequirements(detail: CapabilityReleaseDetailDTO): Record<string, unknown> | undefined {
