...
export class CapabilityReleasePublishService {
  ...
  async bridgeRecorderExport(
    ...
    const draftPayload = {
      ...
      recorderBridge: {
        ...
        guidance:
          typeof dto.exportArtifacts?.guidance === 'string'
            ? dto.exportArtifacts.guidance
            : null,
  ...
  async publishSkill(
    ...
    if (release.sourceType === 'temporal_workflow') {
      ...
      const mappingReadiness = this.capabilityReleaseTemporalSchemaService
        .assessTemporalDocumentMappingReadiness(snapshot.sourcePayload);
    ...
    await accessors.insertAuditEvent(id, 'skill_published', userId, true, `发布 Skill 成功: ${publishedSkillId}`);
    return {
  ...
  private normalizeRecorderPublishPayload(
    ...
    const triggerKeywords = Array.isArray(input.triggerKeywords)
      ...
      : [];
    const paramsSchema =
      input.paramsSchema && typeof input.paramsSchema === 'object' && !Array.isArray(input.paramsSchema)
        ? input.paramsSchema
        : { properties: {}, required: [] };
    ...
    const apiEndpoints =
      input.apiEndpoints && typeof input.apiEndpoints === 'object' && !Array.isArray(input.apiEndpoints)
        ? input.apiEndpoints
        : null;
