...
import type {
  ...
  DevtoolsCliBinary,
  DevtoolsCliSessionState,
  DevtoolsExecResult,
} from './chrome-devtools-cli.types';
import {
  buildPageFingerprint,
  buildScrollScript,
  extractSearchResults,
  matchPageAssertion,
} from './chrome-devtools-cli.parsers';
...
export class ChromeDevtoolsCliAdapter implements BrowserExecutionAdapter {
  ...
  async assertState(dto: AssertBrowserStateDto): Promise<BrowserPageAssertionResultDto> {
    ...
    return {
      matched: matchPageAssertion(dto, pageState, selectorMatched, textMatched),
      pageState,
      details: {
  ...
  private async inspectPageState(sessionId: string): Promise<BrowserPageStateDto> {
    ...
    return {
      ...
      pageFingerprint: buildPageFingerprint(pageUrl, pageTitle),
  ...
  private async handleScroll(
    ...
    const args = [
      'evaluate_script',
      buildScrollScript(direction, this.readOptionalNumberParam(params, ['amount']) ?? 600),
    ];
    const result = await this.execCli(sessionId, args);
    return {
      status: 'success',
  ...
  private async handleListSearchResults(
    ...
    const parsed = extractSearchResults(snapshotText, limit);
    session.lastSnapshotText = snapshotText;
