...
def build_system_prompt(
    ...
    return (
        ...
        "   - Markdown 产物：当用户明确要求输出 `.md` 或 Markdown 文件时，完成内容整理后必须调用 `write_markdown(file_path='...', content='完整 Markdown 正文')`。不要把计划、伪调用文本或 `new_plan` 当作最终回复。\n"
...
def build_user_turn(
    ...
    if re.search(r'(?:\.md\b|\bmarkdown\b|(?:md|markdown)\s*(?:格式)?\s*文件)', prompt, re.I):
        user_parts.append(
            "【Markdown 文件交付契约】:\n"
            "- 本轮必须产生真实、非空的 `.md` 文件，不能只回复计划或口头承诺。\n"
            "- 内容完成后调用 `write_markdown`，参数仅包含安全文件名与完整 Markdown 正文。\n"
            "- 工具成功后再给出简短完成说明；严禁输出 `new_plan`、`web_search(...)` 等内部执行草稿。"
        )
