...
import { TemporalWorkflow } from './temporal-workflow.types';
...
export class TemporalWorkflowController {
  ...
  @ApiOperation({
    summary: 'Generate workflow Python code from saved DSL and persist generated code',
  })
  ...
  @ApiOperation({
    summary: 'Alias of generate-and-save for persisted workflow artifact generation',
  })
  ...
  @ApiOperation({
    summary: 'Compile template-based workflow draft on backend with policy overrides',
  })
  ...
  @ApiOperation({
    summary: 'Generate browser-template workflow draft from browser script or structured commands',
  })
  ...
  @ApiOperation({
    summary: 'Generate AI-assisted workflow draft from natural language and optional URL',
  })
  async generateAiDraft(@Body() data: GenerateAiWorkflowDraftDTO): Promise<AiWorkflowDraft> {
  ...
  async refineAiDraftSession(
    ...
    return this.temporalWorkflowService.refineAiDraftSession(
      ...
      } as RefineAiWorkflowDraftSessionDTO,
      req.user?.id
    );
  }
  ...
  @ApiOperation({
    summary: 'Optimize builtin httpRequest step config with AI and live response preview',
  })
  ...
  @ApiOperation({
    summary: 'Generate builtin structuredTransform step config from real sample and user goal',
  })
  ...
  @ApiOperation({
    summary: 'Alias of validate-saved-artifact for persisted workflow artifact validation',
  })
