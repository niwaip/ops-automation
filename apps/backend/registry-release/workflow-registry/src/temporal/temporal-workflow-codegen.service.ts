...
import { getAiOrchestratorUrl } from './service-endpoints';
...
import {
  AST_GATE_REPAIR_GUIDANCE,
  PYTHON_AST_GATE_SCRIPT,
} from './temporal-workflow-codegen.constants';
...
export class TemporalWorkflowCodegenService {
  ...
  private buildWorkflowCodePrompt(
    ...
    if (activityCodeAlreadyGenerated) {
      lines.push(
        '【本次任务范围】：仅编写 Workflow 胶水代码。Activity 实现已全部提供，不要重复生成或改写。'
      );
    }
    ...
    if (workflowDsl.v2Output?.fields) {
      try {
        ...
        lines.push(
          '此工作流已由 DSL v2Output 声明输出契约。以下由编译器生成的 _build_workflow_result 函数，你必须【原样包含】在 Workflow 类中，严禁修改、删除或简化其中的任何断言：'
        );
  ...
  private formatGate1Errors(errors: PythonGate1Violation[]): string {
    ...
    return errors.map((e) => `第 ${e.line} 行 [${e.code}]: ${e.message}`).join('\n');
  }
