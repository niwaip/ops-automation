...
export class BrowserRecordingExecutionPlanValidatorService {
  normalizePayloadForCompatibility(payload: Record<string, unknown>): Record<string, unknown> {
    ...
    const normalizedLoopDraft = this.normalizeLoopDraftForCompatibility(loopDraft, templateSteps);

    if (normalizedLoopDraft === loopDraft) {
      return payload;
    }

    const nextPayload = { ...payload };
    const nextRuntimeMetadata = { ...runtimeMetadata };
    const nextExecutionPlan = executionPlan ? { ...executionPlan } : undefined;

    if (normalizedLoopDraft) {
      if (nextExecutionPlan) {
        nextExecutionPlan.loopDraft = normalizedLoopDraft;
        nextRuntimeMetadata.executionPlan = nextExecutionPlan;
      }
      nextRuntimeMetadata.loopDraft = normalizedLoopDraft;
    } else {
      if (nextExecutionPlan) {
        delete nextExecutionPlan.loopDraft;
        nextRuntimeMetadata.executionPlan = nextExecutionPlan;
      }
      delete nextRuntimeMetadata.loopDraft;
    }

    if (asRecord(payload.runtimeMetadata)) {
      nextPayload.runtimeMetadata = nextRuntimeMetadata;
    }
  ...
  private validate(
    ...
    const normalizedPayload = options.allowLegacyFallback
      ? this.normalizePayloadForCompatibility(payload)
      : payload;
    ...
    const hasLegacyExecutionFlow =
      Array.isArray(normalizedPayload.executionFlow) && normalizedPayload.executionFlow.length > 0;
    ...
    const missingBranchVariables = this.findMissingBranchVariables(
      templateSteps,
      this.collectKnownVariables(normalizedPayload, executionPlan, templateSteps)
    );
  ...
  private normalizeLoopDraftForCompatibility(
    loopDraft: Record<string, unknown> | undefined,
    templateSteps: Array<Record<string, unknown>>
    ...
    if (asRecord(loopDraft.stopWhen)) {
      return loopDraft;
    }

    const eachIteration = asRecord(loopDraft.eachIteration);
    const hasIterationSteps =
      Array.isArray(eachIteration?.stepIds) &&
      eachIteration.stepIds.some((stepId) => typeof stepId === 'string' && stepId.trim().length > 0);
    const loopRowTemplateStep = templateSteps.find((step) => this.isLoopRowTemplateStep(step));

    if (!hasIterationSteps && !loopRowTemplateStep) {
      return null;
    }

    const derivedStopWhen = this.deriveLoopStopWhen(loopDraft, loopRowTemplateStep);
    if (!derivedStopWhen) {
      return loopDraft;
    }
    ...
    return {
      ...loopDraft,
      stopWhen: derivedStopWhen,
    };
  }

  private isLoopRowTemplateStep(step: Record<string, unknown>): boolean {
    if (this.pickFirstNonEmptyString(step.action) !== 'click') {
      return false;
    }
    const locatorValue = this.pickFirstNonEmptyString(asRecord(step.locator)?.value);
    return Boolean(
      locatorValue &&
        (locatorValue.includes('${rowIndex}') || /^:nth-match\((.+),\s*1\)$/.test(locatorValue))
    );
  }

  private deriveLoopStopWhen(
    loopDraft: Record<string, unknown>,
    loopRowTemplateStep?: Record<string, unknown>
  ): Record<string, unknown> | undefined {
    if (this.pickFirstNonEmptyString(asRecord(loopDraft.target)?.scope) !== 'current_list') {
      return undefined;
    }

    const locator = asRecord(loopRowTemplateStep?.locator);
    const locatorType = this.pickFirstNonEmptyString(locator?.type) || 'css';
    const locatorValue = this.pickFirstNonEmptyString(locator?.value);
    const stopReadLocator = locatorValue
      ? this.toLoopCollectionLocator(locatorValue)
      : undefined;
  ...
  private toLoopCollectionLocator(locatorValue: string): string | undefined {
