...
describe('BrowserCommandService', () => {
  const createService = (modelOverrides?: Partial<{ listModels: jest.Mock; callModel: jest.Mock }>) => {
    ...
    const browserSemanticsClient = {
      resolveRuntimeRuleSet: jest.fn().mockResolvedValue(null),
      createHitLog: jest.fn().mockResolvedValue(undefined),
      createErrorLog: jest.fn().mockResolvedValue(undefined),
    } as unknown as BrowserSemanticsClient;

    return new BrowserCommandService(plannerService, browserSemanticsClient);
  };
  ...
  it('classifies login shadow diff reason when command counts differ', () => {
    const service = createService() as any;

    const diff = service.diffLoginParserResults(
      {
        success: true,
        commands: [{ tool: 'fill', params: { selector: '用户名', value: 'demo' }, description: '填写用户名' }],
        explanation: 'legacy',
      },
      {
        success: true,
        commands: [
          { tool: 'fill', params: { selector: '用户名', value: 'demo' }, description: '填写用户名' },
