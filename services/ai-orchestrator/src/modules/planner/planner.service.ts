        ...
        .map((item) => ({
          ...
          executionFlow: Array.isArray(item.executionFlow)
            ? item.executionFlow
                ...
                .filter(Boolean)
            : [],
          apiEndpoints: (typeof item.apiEndpoints === 'object' && item.apiEndpoints)
    ...
    if (userId) {
      try {
        ...
        if (matchedSkill?.confidence && matchedSkill.confidence > 0) {
          if (matchedSkill.carboneSkillId && (!matchedSkill.executionFlow || matchedSkill.executionFlow.length === 0)) {
