...
export class CapabilityReleaseService implements OnModuleInit {
  ...
  async getReleaseDetail(id: string): Promise<CapabilityReleaseDetailDTO> {
  ...
  async getReleaseCenterDetail(id: string): Promise<CapabilityReleaseDetailDTO> {
    const release = await this.getReleaseOrThrow(id);
    const isVisible =
    ...
    return this.getReleaseDetail(id);
  }
  ...
  async executePublishedSkill(
    ...
    const release = this.mapRelease(releaseRows[0]);
    if (release.sourceType !== 'temporal_workflow') {
      throw new BadRequestException('当前仅支持执行 Temporal Workflow 类型的已发布 Skill');
    }
  ...
  async createRelease(
    ...
    await this.insertAuditEvent(releaseId, 'release_created', userId, true, '创建 Capability Release');
    return this.getReleaseDetail(releaseId);
  }
  ...
  async build(
    ...
    try {
      ...
      if (release.sourceType === 'temporal_workflow') {
        ...
        logs.push(`[${new Date().toISOString()}] 代码生成完成，长度: ${generatedCode.length} 字符`);
      } else {
  ...
  async buildStream(
    ...
    try {
      ...
      if (release.sourceType === 'temporal_workflow') {
        ...
        generatedCode = result.code;
        diffSummary = '已生成 Temporal Workflow Python 代码';
        pushLog(`[${new Date().toISOString()}] 代码生成完成，长度: ${generatedCode.length} 字符`);
      } else {
...
输出格式 (JSON)：
{
  ...
  private async ensureInfrastructure(): Promise<void> {
    ...
    this.logger.log('Capability Release infrastructure ensured');
  }
  ...
  private async getReleaseOrThrow(id: string): Promise<CapabilityReleaseDTO> {
    ...
    if (!rows[0]) {
      throw new NotFoundException('Capability Release 不存在');
    }
