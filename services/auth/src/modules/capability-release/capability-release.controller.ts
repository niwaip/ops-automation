...
export class CapabilityReleaseController {
  ...
  async getReleaseCenterDetail(@Param('id') id: string) {
    const release = await this.capabilityReleaseService.getReleaseCenterDetail(id);
    return { release };
  }
  ...
  async getRelease(@Param('id') id: string) {
    const release = await this.capabilityReleaseService.getReleaseDetail(id);
    return { release };
  }

  @Delete(':id')
  @Roles('admin')
  async archiveRelease(@Param('id') id: string, @Request() req: any) {
    return this.capabilityReleaseService.archiveRelease(id, req.user?.id);
  }

  @Post()
  @Roles('admin')
  async createRelease(@Body() body: CreateCapabilityReleaseDTO, @Request() req: any) {
    const release = await this.capabilityReleaseService.createRelease(body, req.user?.id);
    return { release };
  }
