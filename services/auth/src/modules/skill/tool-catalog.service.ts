...
export class ToolCatalogService implements OnModuleInit {
  ...
  async getCatalogItem(name: string): Promise<ToolCatalogDTO> {
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT
         tc.*,
         COALESCE(usage.bound_skill_count, 0) AS bound_skill_count,
  ...
  async updateCatalogItem(name: string, dto: UpdateToolCatalogDTO): Promise<ToolCatalogDTO> {
  ...
  async getCatalogItemsByNames(toolNames: string[]): Promise<Map<string, ToolCatalogDTO>> {
  ...
  private mapTool(row: any): ToolCatalogDTO {
    return {
      id: String(row.id),
      ...
      usageSummary: {
        boundSkillCount: Number(row.bound_skill_count || 0),
        boundSkillNames: Array.isArray(row.bound_skill_names)
          ? row.bound_skill_names.map((name: unknown) => String(name))
          : undefined,
        boundSkills: Array.isArray(row.bound_skills_json)
          ? row.bound_skills_json.map((skill: any) => ({
              id: String(skill.id),
              name: String(skill.name),
              isActive: Boolean(skill.isActive),
              configStatus: skill.configStatus ? String(skill.configStatus) : undefined,
              isPublished: Boolean(skill.isPublished),
              publishedReleaseStatus: skill.publishedReleaseStatus
                ? String(skill.publishedReleaseStatus)
                : null,
              publishedDeploymentStatus: skill.publishedDeploymentStatus
                ? String(skill.publishedDeploymentStatus)
                : null,
            }))
          : undefined,
      },
