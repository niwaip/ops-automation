...
> 说明
>
> - 本文中的 Office 宿主抽象、`HostAdapter`、`DocumentIR`、前端分层与多宿主能力规划仍可作为参考。
> - 对于 Word 模板化场景中的“真实样本上传 -> 模板对比 -> AI 全局理解 -> 用户补充语言 -> 按章节/块 AI 参数识别 -> 参数应用与模板产出”主链路，应以 `doc/Template_Advanced_Scenarios.md` 的最新设计为准。
> - 若本文与上述最新设计在识别主流程优先级上存在冲突，应以后者为准，不再按“规则优先、AI 仅增强”的旧口径实现 Word 主流程。
...
识别主链路应由 AI 驱动，规则层负责提示增强、结果校验和失败回退，而不是先由规则产出主结果：

1. 先基于结构化文档和候选上下文做 AI 全局理解，统一业务主题、章节语义和命名口径。
2. 再按章节、块、对照组等可解释单元循环调用 AI，直接生成字段建议、变量建议、循环建议和歧义消解结果。
3. 规则引擎负责提供稳定结构信号，并在高风险字段、术语命中、枚举命中、金额/日期/账号等场景做强约束校验。
4. 仅当单块或整批 AI 调用失败时，才回退到规则识别或启发式结果。

规则层仍应保留以下确定性能力，作为 prompt 输入、校验条件和 fallback 来源：
...
在 Word 模板化场景下，AI 还应承担：

- 基于真实样本与候选池的全局理解
- 按章节/块完成字段识别与命名归一
- 结合术语、锚点和版式线索完成跨语言关系判断
