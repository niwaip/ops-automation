...
| :--- | :--- | :--- | :--- | :--- |
| `apps/backend/core/platform/src/modules/auth` | 认证、登录、权限入口 | `governance` | `identity-access` | `auth.service`、`auth.controller`、`auth.module` 与 auth 请求/response 契约已迁入 `identity-access`；旧平台目录与 `src/dto/*` DTO 包装已删除，`platform` 当前仅保留 `src/governance/identity-access/*` runtime bridge 绑定 |
| `apps/backend/core/platform/src/modules/organization` | 组织、部门、归属 | `governance` | `organization` | `organization.service`、`organization.controller`、`organization.module`、组织请求 DTO 与 response 契约已迁入 `governance/organization`；旧平台目录与 `src/dto/*` DTO 包装已删除，`platform` 当前仅保留 `src/governance/organization/*` runtime bridge 绑定 |
| `apps/backend/core/platform/src/modules/user` | 用户信息管理 | `governance` | `identity-access` | 用户查询、角色变更、启停用主体与 `user.controller` / `user.module` 已迁入 `identity-access`；旧平台目录与 `src/dto/*` DTO 包装已删除，`platform` 当前仅保留 `src/governance/identity-access/*` runtime bridge 绑定 |
| `apps/backend/core/platform/src/guards/*` | 访问控制守卫 | `governance` | `identity-access` | `RolesGuard`、`JwtAuthGuard`、`RbacGuard` 已归位；旧平台目录已删除，相关消费方统一切到 `@ops/identity-access` |
| `apps/backend/core/platform/src/decorators/*` | 权限与角色装饰器 | `governance` | `identity-access` | 已完成物理归位；旧平台目录已删除，相关消费方统一切到 `@ops/identity-access` |
| `apps/backend/core/platform/src/strategies/*` | JWT / LDAP 等策略 | `governance` | `identity-access` | `ldap.strategy`、`jwt.strategy` 已归位；旧平台目录已删除，相关消费方统一切到 `@ops/identity-access` |
| `apps/backend/core/platform/src/modules/skill` | Skill 管理、匹配、绑定、校验 | `registry-release` | `skill-registry` | 已在 `apps/backend/registry-release/skill-registry/README.md` 中显式纳入统一逻辑视图，并继续按 `registry / binding / access / matching / enrichment / validation` 收敛；`core/platform/src/skill-registry/*` 纯 facade 与 `modules/skill/{access,binding,matching,enrichment,validation}` 分组 `index.ts` compat 壳已在后续 Phase E 删除；包侧 `apps/backend/registry-release/skill-registry/src/access|binding|enrichment|matching|validation` 五个零消费者叶子入口也已在后续 Phase E 删除，当前仅保留 `registry/index.ts` 作为最小逻辑视图占位，平台消费方已直接切到 `modules/skill/*` 根入口或真实实现文件 |
| `apps/backend/core/platform/src/modules/execution-flow` | 流程模板定义 | `registry-release` | `workflow-registry` | 已在 `apps/backend/registry-release/workflow-registry/README.md` 中显式纳入统一逻辑视图，并已补充 `flow-template/README.md`、`validation/README.md` 说明 Flow 模板注册面、模板服务以及验证结果类型与验证 facade 的统一边界；`core/platform/src/workflow-registry/flow-template` 纯 facade、`modules/execution-flow/template/index.ts` compat 壳、`modules/execution-flow/registry/index.ts` 过渡 barrel 以及包侧 `apps/backend/registry-release/workflow-registry/src/execution-flow.ts` 纯转发 leaf shell 已在后续 Phase E 删除，平台与包侧消费方已直接切到 `modules/execution-flow/*` 真实实现、`@ops/platform/dist/modules/execution-flow/*` 或真实实现文件 |
| `apps/backend/core/platform/src/modules/temporal-workflow` | Workflow / Activity 模板、代码生成、草稿 | `registry-release` | `workflow-registry` | 已在 `apps/backend/registry-release/workflow-registry/README.md` 中显式纳入统一逻辑视图，并继续按 `workflow / activity / codegen / validation` 收敛，且其校验服务与类型出口已统一纳入 `validation/README.md` 说明；`core/platform/src/workflow-registry/{activity-template,browser-bridge}` 纯 facade、`modules/temporal-workflow/{workflow,validation,codegen}` 下一层一行转发 `*.service.ts` compat 壳、根目录下 `temporal-workflow-browser-draft.service.ts` / `temporal-workflow-browser.helpers.ts` / `temporal-activity-execution.service.ts` / `temporal-activity-execution.helpers.ts` 转发壳、`temporal-workflow-service.utils.ts` / `temporal-workflow-codegen.helpers.ts` 聚合 wrapper、包侧 `workflow-template.services.ts` / `workflow-registry-validation.ts` / `workflow-registry-codegen.ts` 二跳 wrapper，以及 `apps/backend/registry-release/workflow-registry/src/temporal-workflow.ts`、`apps/backend/registry-release/workflow-registry/src/temporal-activity.ts`、`apps/backend/registry-release/workflow-registry/src/browser-bridge/index.ts`、`modules/temporal-workflow/index.ts`、`modules/temporal-workflow/activity/index.ts`、`modules/temporal-workflow/workflow/index.ts`、`modules/temporal-workflow/validation/index.ts`、`modules/temporal-workflow/codegen/index.ts`、`modules/temporal-workflow/runtime-bridge/index.ts`、`core/platform/src/workflow-registry/workflow-template/index.ts` 与 `core/platform/src/workflow-registry/validation/index.ts` barrel 壳已在后续 Phase E 删除，平台与包侧消费方已直接切到 `workflow-registry/*` 真实实现、`core/platform` 下 `browser-bridge` 真实目录、`runtime-bridge/*` 真实实现或具体 activity/utils/helper 文件 |
| `apps/backend/core/platform/src/release-manager` | 发布、编译、校验、运行时绑定 | `registry-release` | `release-manager` | 已在 `apps/backend/registry-release/release-manager/README.md` 中显式纳入统一逻辑视图，并已补充 `release/README.md`、`compiler/README.md`、`validator/README.md`、`publisher/README.md`、`audit/README.md` 说明发布主入口、构建装配、发布前校验、发布后桥接与审计收口边界；旧 `modules/capability-release` 现仅保留兼容层与运行时适配锚点，根层 `*.service.ts` / `*.mapper.ts` 与五个子层都不再持有真实实现 |
...
| `apps/backend/domain/browser-template/*` | 浏览器模板管理、编译、校验 | `capabilities` | `browser-domain/templates` | 真实运行包根目录已迁到 `apps/backend/capabilities/browser-domain/templates`；已明确“设计时模板资产 + 本地过渡发布接口”边界，`publish / deprecate / revoke` 后续收敛到 `release-manager`；旧目录 `src/` 已移除，当前仅保留历史 README |
| `apps/backend/domain/browser-semantics/*` | 浏览器语义规则、发布、运行时 | `capabilities` | `browser-domain/semantics` | 历史旧运行包路径，真实运行根已迁到 `apps/backend/capabilities/browser-domain/semantics`，且旧物理路径已在后续收口中完成删除 |
...
| `apps/backend/domain/document-engine/*` | 文档模板、渲染、Studio、参数发现 | `capabilities` | `document-domain` | 已明确承接 `template / render / runtime-facade` 逻辑层，并在正式渲染结果中补充 `artifacts: ArtifactRef[]`；compose/test 入口与历史 `carbone-engine` 默认 `typecheck/build/test/test:e2e/migrate:sidecar-to-db` 脚本已收口到 `apps/backend/capabilities/document-domain`，兼容包名由 `carbone-engine-compat` 承接，旧目录 package manifests、`src/`、`prisma/` 与 `tsconfig.json` 已移除，当前仅保留历史 README |
| `apps/backend/capabilities/document-domain/report/*` | 报表分析、导出、模板生成 | `capabilities` | `document-domain/report` | 已成为 `report` 的真实运行包根目录，局部复用 `template / render` 逻辑层，并在报表结果中补充 `artifacts: ArtifactRef[]` |
| `apps/backend/domain/report/*` | 报表旧运行包残留目录 | `capabilities` | `document-domain/report` | 历史旧运行包路径，真实运行根已迁到 `apps/backend/capabilities/document-domain/report`，且旧物理路径已在后续收口中完成删除 |
...
| `apps/backend/runtimes/sandbox-worker/*` | 动态代码沙箱执行与校验 | `runtimes` | `sandbox-worker` | 已完成独立目录承接，并纳入 `apps/backend/runtimes/README.md` 的统一运行时视图；默认 URL / task queue 命名已切到 `SANDBOX_WORKER_URL` / `SANDBOX_WORKER_TASK_QUEUE`，旧命名仅保留兼容读取 |
...
- 仍保留少量历史兼容命名，例如 `SANDBOX_AGENT_URL`、`TEMPORAL_SANDBOX_AGENT_URL`、`SANDBOX_TASK_QUEUE` 与 `sandbox-agent-task-queue`；当前它们仅用于兼容读取，不再作为 compose 默认输出。
