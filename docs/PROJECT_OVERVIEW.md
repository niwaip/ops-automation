...
│   │   ├── document-domain/ # 文档模板、Studio 与渲染能力
│   │   ├── document-report/ # 报表任务 API、分析与结果分发
│   │   └── carbone-engine-compat/ # 历史命令兼容入口
│   │
│   ├── runtimes/            # [执行器] 物理运行容器
│   │   ├── browser-worker/  # Playwright 驱动、动作执行、DOM 快照抽取
│   │   ├── replay-worker/   # 轨迹回放引擎
│   │   ├── sandbox-worker/  # 沙箱任务执行
│   │   ├── temporal-worker/ # Temporal 工作单元执行
│   │   └── personal-sandbox-runner/ # 个人沙箱源码、插件和技能资产
│   │
...
├── office-addin/            # Office 模板设计与 AI 辅助插件
├── ai-passport-agent/       # ESP32-C3 随身语音任务终端固件
├── desktop/                 # 桌面端宿主脚手架
├── mobile/                  # 移动端宿主脚手架
└── social-platform/         # 社交平台宿主脚手架

packages/                    # 跨服务契约、能力 SDK 与多端 user-core
builtin-skills/              # 平台内置技能发布资产
database/                    # Schema 属主、迁移权威与数据库安全策略
docker/                      # 镜像和 Compose 装配，不作为业务源码归属地
tests/                       # 跨服务验收、契约、集成和端到端测试
```
...
工作模式接收聊天窗口上传的图片时，按附件 MIME 类型识别视觉输入。无需外部工具的图片理解请求直接使用多模态模型消息；需要搜索、生成文件或其他工具的请求进入规划链。LLM Operation 的运行时输入只包含其冻结 Schema 声明的字段，规划上下文和附件元数据不能作为额外业务字段传入封闭输入契约。

IM 长连接由渠道运行时负责自动恢复。微信轮询遇到暂时性网络错误时按指数退避重新建立连接，凭据无法解密或明确失效时进入 `reauth_required`；主动停用、解绑或服务关闭会取消待执行的重连。小智连接器通过数据库租约持有出站 WebSocket，并在断线后按退避策略重新连接。
...
| **数据库与持久化**    | PostgreSQL 15, Prisma ORM (v5.22.0)        | 单一 Prisma 权威治理，当前 94 张物理表均声明唯一属主 |
...
4. **依赖一致性保证**：
   ...
   - `pnpm-workspace.yaml` 是唯一 Workspace 范围事实源；内部 `@ops/*` 依赖统一使用 `workspace:*`。
   - `pnpm run check:architecture` 校验包名唯一性、内部依赖解析、循环依赖、嵌套锁文件与旧后端平面回流。
