...
const DEBT_EXEMPT_FILES = new Map([
  ['database-design.schema.sql', 'declarative database schema asset'],
]);

const violations = [];
const warnings = [];
const debtExemptions = [];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      try {
        ...
        const debtReason = DEBT_EXEMPT_FILES.get(name);

        if (debtReason) {
          if (lines > WARN_LINE) {
            debtExemptions.push({ lines, path: relPath, reason: debtReason });
          }
          continue;
        }
...
if (debtExemptions.length > 0) {
  console.log(`\n[DEBT] ${debtExemptions.length} explicitly tracked oversized-file exemption(s):`);
  debtExemptions.sort((a, b) => b.lines - a.lines);
  for (const item of debtExemptions) {
    console.log(`  - ${String(item.lines).padStart(4)} lines: ${item.path} (${item.reason})`);
  }
}
